...
from .database import (add_history_entry, queue_downgrade,
                       get_due_downgrades, update_downgrade_status)
...
async def run_sync():
    ...
    try:
        ...
        # Build Plex added-date and resolution maps if needed
        plex_added_map: dict[int, datetime] = {}
        plex_4k_map: dict[int, bool] = {}
        dg = config.downgrade
        if dg and dg.enabled and dg.date_source == "plex":
            plex_added_map, plex_4k_map = await _build_plex_added_map(config)

        # ── Execute due downgrades first ──────────────────────────────────────
        if dg and dg.enabled:
            await _execute_due_downgrades(radarr, profile_map, stats)
...
async def _build_plex_added_map(config) -> tuple[dict[int, datetime], dict[int, bool]]:
    """
    Fetch local Plex library and build:
      - tmdb_id → addedAt (datetime)
      - tmdb_id → is_4k (bool)
    Returns (date_map, is_4k_map). Both empty dicts on failure.
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30, verify=False) as client:
            ...
            for section in [s for s in sections if s.get("type") == "movie"]:
                ...
                for item in r2.json().get("MediaContainer", {}).get("Metadata", []):
                    ...
                    if not added_at:
                        continue
                    # Determine if any media part is 4K
                    resolution = ""
        ...
        logger.info(f"[Downgrade] Plex map: {len(date_map)} movies, {is_4k_count} are 4K")
        if date_map:
...
async def _execute_due_downgrades(radarr: RadarrClient, profile_map: dict, stats: dict):
    ...
    for d in due:
        try:
            ...
            if d.radarr_file_id and d.radarr_file_id > 0:
                await radarr.delete_movie_file(d.radarr_file_id)
                logger.info(f"  [Downgrade] EXECUTED '{d.movie_title}' — file deleted, profile → '{d.target_profile}'")
            else:
                logger.info(f"  [Downgrade] EXECUTED '{d.movie_title}' — profile → '{d.target_profile}' (no file)")
            ...
            await add_history_entry(d.source_id, d.source_name, d.movie_title, d.tmdb_id,
                                    "downgraded",
                                    f"'{d.current_profile}' → '{d.target_profile}' + file deleted")
        except Exception as e:
